...
void CompilationInfoDumper::DumpLoweredFunc() {
  ...
  for (int idx = 0; idx < info_.lowered_funcs.size(); idx++) {
    ...
    Dump(FLAGS_cinn_dump_group_lowered_func, idx, "lowered_function.txt", content.str());
  }
}
...
void CompilationInfoDumper::DumpSourceCode() {
  ...
  for (int idx = 0; idx < info_.source_codes.size(); idx++) {
    Dump(FLAGS_cinn_dump_group_source_code, idx, "source_code.cu", info_.source_codes[idx]);
  }
}
...
void CompilationInfoDumper::DumpPtxCode() {
  ...
  for (int idx = 0; idx < info_.source_ptxs.size(); idx++) {
    Dump(FLAGS_cinn_dump_group_ptx, idx, "source_ptx.ptx", info_.source_ptxs[idx]);
  }
}
...
void CompilationInfoDumper::DumpInstruction() {
  ...
  for (int idx = 0; idx < info_.instructions.size(); idx++) {
    Dump(FLAGS_cinn_dump_group_instruction, idx, "instruction.txt", info_.instructions[idx]->DumpInstruction());
  }
}
...
void CompilationInfoDumper::Dump(const std::string& base_path,
  ...
  auto dump_path = utils::StringFormat("%s/fusion_group_%d", base_path.c_str(), idx);
  if (!hlir::framework::MakeDirectory(dump_path, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
    LOG(WARNING) << "Failed to make directory: \"" << dump_path << "\", the instruction for this group will not dump.";
  } else {
    auto dump_file = utils::StringFormat("%s/%s", dump_path.c_str(), file_name.c_str());
