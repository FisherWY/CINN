...
void DumpCompilationInfo::DumpLoweredFunc() {
  ...
  for (int idx = 0; idx < info_.lowered_funcs.size(); idx++) {
    auto dump_path = utils::StringFormat("%s/fusion_group_%d", FLAGS_cinn_dump_group_lowered_func.c_str(), idx);
    if (!hlir::framework::MakeDirectory(dump_path, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
      LOG(WARNING) << "Failed to make directory: \"" << dump_path
                   << "\", the lowered functions for this group will not dump.";
    } else {
      auto dump_file = utils::StringFormat("%s/%s", dump_path.c_str(), "lowered_function.txt");
...
void DumpCompilationInfo::DumpSourceCode() {
  ...
  for (int idx = 0; idx < info_.source_codes.size(); idx++) {
    auto dump_path = utils::StringFormat("%s/fusion_group_%d", FLAGS_cinn_dump_group_source_code.c_str(), idx);
    if (!hlir::framework::MakeDirectory(dump_path, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
      LOG(WARNING) << "Failed to make directory: \"" << dump_path
                   << "\", the source code for this group will not dump.";
    } else {
      auto dump_file = utils::StringFormat("%s/%s", dump_path.c_str(), "source_code.cu");
...
void DumpCompilationInfo::DumpPtxCode() {
  ...
  for (int idx = 0; idx < info_.source_ptxs.size(); idx++) {
    auto dump_path = utils::StringFormat("%s/fusion_group_%d", FLAGS_cinn_dump_group_ptx.c_str(), idx);
    if (!hlir::framework::MakeDirectory(dump_path, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
      LOG(WARNING) << "Failed to make directory: \"" << dump_path << "\", the source ptx for this group will not dump.";
    } else {
      auto dump_file = utils::StringFormat("%s/%s", dump_path.c_str(), "source_ptx.ptx");
...
void DumpCompilationInfo::DumpInstruction() {
  ...
  for (int idx = 0; idx < info_.instructions.size(); idx++) {
    auto dump_path = utils::StringFormat("%s/fusion_group_%d", FLAGS_cinn_dump_group_instruction.c_str(), idx);
    if (!hlir::framework::MakeDirectory(dump_path, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
      LOG(WARNING) << "Failed to make directory: \"" << dump_path
                   << "\", the instruction for this group will not dump.";
    } else {
      auto dump_file = utils::StringFormat("%s/%s", dump_path.c_str(), "instruction.txt");
