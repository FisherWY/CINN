...
BuildResult SimpleBuilder::Build(const MeasureInput& input) {
  ...
  GraphCompiler::CompilationContext& context = graph_compiler_->GetCompilationContext();
  ...
  VLOG(5) << "call GraphCompiler to Build with Graph::Group size=" << context.groups.size()
          << ", lowered_funcs group size=" << context.lowered_funcs.size();
  GraphCompiler::CompilationResult compiled_result = graph_compiler_->Build(&context);
