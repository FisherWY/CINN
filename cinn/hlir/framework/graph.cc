...
void Graph::VisualizeGroupedGraph(const std::vector<std::vector<Node*>>& origin_groups,
  ...
  LOG(INFO) << "Dump graph debug info to: " << FLAGS_cinn_fusion_groups_graphviz_dir;
  ...
  for (int idx = 0; idx < groups.size(); idx++) {
    ...
    auto group_path = utils::StringFormat("%s/fusion_group_%d", FLAGS_cinn_fusion_groups_graphviz_dir.c_str(), idx);
    if (!MakeDirectory(group_path, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
      LOG(WARNING) << "Failed to make directory: \"" << group_path << "\", skip dump info for this group.";
      continue;
    }
    // Create test_group_x.py
    auto python_test_file = utils::StringFormat("%s/test_group_%d.py", group_path.c_str(), idx);
    WriteToFile(python_test_file, GenerateGroupPythonCode(groups[idx], fetch_var_ids));
    // Create x_group_name.dot
    auto graph_group_file = utils::StringFormat("%s/graph_group_%d.dot", group_path.c_str(), idx);
  ...
  // Grouped graph
  auto grouped_graph_file = utils::StringFormat("%s/grouped_graph.dot", FLAGS_cinn_fusion_groups_graphviz_dir.c_str());
