...
TEST(GraphCompilerTest, TestInsertBufferHandlers) {
  ...
  auto runtime_program_disable = gc_disable.Build(&context_disable).runtime_program;
  ...
  auto runtime_program_enable = gc_enable.Build(&context_enable).runtime_program;
...
TEST(GraphCompilerTest, TestLowering) {
  ...
  LOG(INFO) << "Runtime program status: " << (result.runtime_program.get() == nullptr);
