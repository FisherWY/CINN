...
class GraphCompiler final {
 public:
  ...
  CompilationResult Lowering(const CompileOptions& options,
                             std::unordered_set<std::string>&& fetch_var_ids = {},
                             void* stream                                    = nullptr);

  CompilationResult CodegenAndJit(const CompileOptions& options,
                                  std::unordered_set<std::string>&& fetch_var_ids = {},
                                  void* stream                                    = nullptr);

  CompilationResult BuildInstruction(const CompileOptions& options,
                                     std::unordered_set<std::string>&& fetch_var_ids = {},
                                     void* stream                                    = nullptr);
